...
// 認証が必要なAPIのパターンを拡張・最適化
const AUTH_REQUIRED_PATTERNS = [
  '/api/events/find?event_id=',  // SNS認証投票用（最重要）
  '/api/events/vote',            // SNS認証投票用
  '/api/auth/',                  // 認証API
  '/api/events/find',            // すべてのfind API（安全側に寄せる）
];

// URLが認証を必要とするかチェック（より確実な判定）
const requiresAuth = (url) => {
  // event_idパラメータがある場合は必ず認証必要と判定
