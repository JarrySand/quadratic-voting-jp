...
export const useVoteManager = (query) => {
  ...
  const calculateShow = useCallback((current, increment) => {
    ...
    // 投票期間チェック（クライアント側でもガード）
    const startDate = data.event_data.start_event_date ? moment(data.event_data.start_event_date) : null;
    const endDate = data.event_data.end_event_date ? moment(data.event_data.end_event_date) : null;
    const now = moment();
