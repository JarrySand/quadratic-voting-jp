...
export async function getAuthContext(req) {
  try {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.log("🔍 [DEBUG] getAuthContext 開始:", {
        method: req.method,
        query: req.query,
        body: req.body,
        headers: {
    ...
    if (req.body && req.body.id) {
      if (process.env.NODE_ENV === 'production') {
        console.log("🔍 [DEBUG] 個別投票認証 (POST body):", {
          user_id: req.body.id,
    ...
    if (req.query && req.query.id) {
      if (process.env.NODE_ENV === 'production') {
        console.log("🔍 [DEBUG] 個別投票認証 (GET query):", {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.log("🔍 [DEBUG] NextAuth.js トークン取得結果:", {
        token_exists: !!token,
    ...
    if (!token) {
      ...
      if (process.env.NODE_ENV === 'production') {
        console.error("🔍 [DEBUG] NextAuth.js トークン取得失敗:", {
          error_message: errorMessage,
          cookies_present: !!req.headers.cookie,
          cookie_content: req.headers.cookie?.split(';').map(c => c.trim()).filter(c => c.includes('next-auth')),
          timestamp: new Date().toISOString()
        });
      }
    ...
    if (!providerId) {
      ...
      if (process.env.NODE_ENV === 'production') {
        console.error("🔍 [DEBUG] プロバイダーID取得失敗:", {
          error_message: errorMessage,
    ...
    if (process.env.NODE_ENV === 'production') {
      console.log("🔍 [DEBUG] 認証コンテキスト作成成功:", {
    ...
    if (process.env.NODE_ENV === 'production') {
      console.error("🔍 [DEBUG] getAuthContext エラー:", {
        error_message: errorMessage,
